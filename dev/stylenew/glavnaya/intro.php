<!--Главный экран intro. Слайдеры и баннеры -------------------------------------------------->		
<section id="intro" class="bg-grey">
<div class="container block-size">
         
<!--большой слайдер-------------------------------------------------->         
<div class="row">
<div class="blockBorder inc-m-15">
<div class="col-sm-12 col-md-8 p-0">
<div class="introSlider">

						
<!-------->
<div class="owl-carousel leftIntro">

<!-- card -->
<div class="item" style="background: url(images/slider/slider1.jpg) 0% 0% / cover no-repeat;">

<div class="row">
<div class="col-xs-12 col-7 slideText">
<h2><a href="/roznichnaya-set?view=article&amp;id=39&amp;catid=17">Аккумуляторы от первого поставщика</a><br>
20 лет продаж</h2>
<hr>
<p>Быстро
подберем / доставим / установим</p>
</div>

</div>
</div>



<!-- card -->
<div class="item" style="background: url(images/slider/slider2.jpg) 0% 0% / cover no-repeat;">

<div class="row">
<div class="col-xs-12 col-7 slideText">
<img src="images/logo/logo-q8.png" alt="">
<h2>
<a href="/roznichnaya-set?view=article&amp;id=40&amp;catid=17">Автомобильные масла из Бельгии</a></h2>
<hr>
<p>Гарантия
оригинального качества</p>
</div>

</div>
</div>



<!-- card -->
<div class="item" style="background: url(images/slider/slider3.jpg) 0% 0% / cover no-repeat;">
<div class="row">
<div class="col-xs-12 col-7 slideText">

<h2><a href="/sdat-staryj-akkumulyator">Бесплатная диагностика аккумуляторов</a></h2>
<hr>
<p>
Купим отработанные аккумуляторы.<br>
Собственные сервисные центры
</p>
</div>


</div>
</div>


</div>
<!-------->
	

</div>
</div>
            
<div class="col-sm-12 col-md-4 rightIntro p-0 hidden-xs">           
<!--banner1-------------------------------------->                         
<div class="p-2" style="height: 189px; overflow: hidden; border-bottom:1px solid #ebebeb; text-align: center; vertical-align: middle;">

<a href="/aktsii-skidki" class="col-sm-12 col-md-6 bg-red" style="min-height: 100%; display: inline-block; color: #FFFFFF; text-transform: uppercase">
<span style="display: block; min-height: 100%; position: absolute; top: 75%; bottom: 0; left: 0; right: 0; margin: auto;">
Наши<br>
Акции
</span> 
</a>
<a href="/sdat-staryj-akkumulyator" class="col-sm-12 col-md-6 bg-black" style="min-height: 100%; display: inline-block; color: #FFFFFF; text-transform: uppercase"> 
<span style="display: block; min-height: 100%; position: absolute; top: 75%; bottom: 0; left: 0; right: 0; margin: auto;"> 
Сдать старый<br>
аккумулятор
</span>
</a>
</div>
<!--//banner1-------------------------------------------> 
 
 
<!--banner2-------------------------------------->                         
<div class="bannerSlider">
<!-------->
<div class="owl-carousel">

<!-- card -->
<div class="item">
<div class="row">

<i class="fas fa-certificatehtml" style="position: absolute; color:#E9522C; right: 20px; top: 20px; font-size:65px;">
<span style="position: absolute; top:22px; right:15px; color: #FFFFFF; font-size: 14px; font-family:Arial; letter-spacing: -1px;">-<strong style=" font-size: 22px;">10</strong>%</span>	
</i>
<div class="col-xs-12">
<h4 style="font-size: 1.4rem; text-transform:uppercase; font-weight: bold">Купи прямо сейчас!</h4> 
</div>
<br style="clear:both">


<div class="col-xs-4">
<img src="/images/virtuemart/product/resized/akom-asia-90-evro-1_0x90.jpg" alt="akom-asia-90-evro" style="width:100%;">
</div>



<div class="col-xs-8 py-1" style="">
<strong style="color: #fff; background: #E9522C; padding:1px 2px;">Горячее предложение </br>Скидка 10%</strong>
<h4 style="font-weight: bold; margin-bottom: 1.0em;"><a href="/catalog/akkumulators/akom/akom-asia-90-evro">АКОМ-ASIA 90 ЕВРО</a></h4>

	<p>				
	<div class="prodText">
						
						Ёмкость (А/ч)-90<br>Ток холодной прокрутки А(EN)-800<br><span class="product-size">305x173x225 мм</span>						<span class="redPrice">
												</span>
					</div>
</p> 
</div>


<div class="col-xs-12">
<button type="submit" class="btn btn-default text-center bg-red" style="border:0; color: #FFFFFF; text-shadow:0">Предложение действительно до 31.01.20</button>
</div>

</div>
</div>





                
</div>
<!-------->

</div>
<!--//banner2------------------------------------------->                       
</div>
<!--большой слайдер-------------------------------------------------->         
         
          
<div class="row bannerCatalog">
<div class="col-sm-12 py-4 px-3" style="border-top: 1px solid #ebebeb; overflow: hidden;">         

<!--------->
<div class="row">
<div class="col-md-3 text-center hidden-xs">
<a href="/catalog/akkumulators/" class="title">Наш каталог</a><br>
<a href="/catalog/akkumulators/" class="button">Перейти</a>
</div>

<div class="col-md-3 col-xs-12">
<a href="/catalog/akkumulators/" class="active">
<div class="bannerImg" style="background: url(images/banner/banner-akb1.jpg) bottom right / cover no-repeat;">
</div>
<p>Купить аккумулятор</p>
</a>
</div>

<div class="col-md-3 col-xs-12">
<a href="/catalog/q8/">
<div class="bannerImg" style="background: url(images/banner/banner-q81.jpg) bottom right / cover no-repeat;">
</div>
<p>Купить масло Q8</p>
</a>
</div>

<div class="col-md-3 col-xs-12">
<a href="/catalog/zaryadnye-ustrojstva-i-aksessuary/">
<div class="bannerImg" style="background: url(images/banner/banner-dlaakb.jpg) bottom right / cover no-repeat;">
</div>
<p>Купить аксессуары</p>
</a>
</div>

<div class="col-xs-12 hidden-md hidden-lg">
<a href="/sdat-staryj-akkumulyator">
<div class="bannerImg" style="background: url(images/banner/banner-dlaakb.jpg) bottom right / cover no-repeat;">
</div>
<p>Сдать аккумулятор</p>
</a>
</div>

</div>
<!--------->

</div>
</div>
      
</div>  
       
                  
                 
      
                          
          
</div>          
</div>
</section>	
<!--//Главный экран intro. Слайдеры и баннеры -------------------------------------------------->
