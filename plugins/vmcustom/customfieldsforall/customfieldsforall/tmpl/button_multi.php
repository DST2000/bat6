...
foreach ($options as $key=>$v) {
	...
	<div class="inline-control-group">
		<input type="checkbox" value="<?php echo $v->id ?>"
			id="<?php echo $input_id?>" class="cf4all_checkbox"
			name="<?php echo $field_name?>[customsforall_option<?php echo $key;?>]" />
		<label class="cf4all_button cf4all_no_color_btn <?php echo $class?>"
			for="<?php echo $input_id?>" <?php echo $title ?>><?php echo $label?></label>
	</div>
	<?php
}?>
