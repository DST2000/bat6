...
/**
 * Methods supporting a list of batbrand records.
 *
 * @since  1.6
 */
class BatbrandsModelClients extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$defaultPurchase = JComponentHelper::getParams('com_batbrands')->get('purchase_type', 3);
		...
		$query->from($db->quoteName('#__batbrand_clients') . ' AS a');

		// Join over the batbrands for counting
		$query->select('COUNT(b.id) as nbatbrands')
			->join('LEFT', '#__batbrands AS b ON a.id = b.cid');
	...
	public function getItems()
	{
		...
		// Faster to do three queries for very large batbrand trees.
		...
		// Get the published batbrands count.
		$query = $db->getQuery(true)
		...
		// Get the unpublished batbrands count.
		$query->clear('where')
			->where('state = 0')
			->where('cid IN (' . $clientIds . ')');
		$db->setQuery($query);
		...
		// Get the trashed batbrands count.
		$query->clear('where')
			->where('state = -2')
			->where('cid IN (' . $clientIds . ')');
		$db->setQuery($query);
		...
		// Get the archived batbrands count.
		$query->clear('where')
			->where('state = 2')
			->where('cid IN (' . $clientIds . ')');
		$db->setQuery($query);
