 ...
 * @since  1.6
 */
class BatbrandsModelTracks extends JModelList
{
	/**
	 * The base name
	 *
	...
	protected function populateState($ordering = 'b.name', $direction = 'asc')
	{
		...
		// Load the parameters.
		$this->setState('params', JComponentHelper::getParams('com_batbrands'));
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName(array('a.track_date', 'a.track_type', 'a.count')))
			->select($db->quoteName('b.name', 'batbrand_name'))
			->select($db->quoteName('cl.name', 'client_name'))
			->select($db->quoteName('c.title', 'category_title'));

		// From tracks table.
		$query->from($db->quoteName('#__batbrand_tracks', 'a'));

		// Join with the batbrands.
		$query->join('LEFT', $db->quoteName('#__batbrands', 'b') . ' ON ' . $db->quoteName('b.id') . ' = ' . $db->quoteName('a.batbrand_id'));

		// Join with the client.
		$query->join('LEFT', $db->quoteName('#__batbrand_clients', 'cl') . ' ON ' . $db->quoteName('cl.id') . ' = ' . $db->quoteName('b.cid'));
		...
		// Filter by search in batbrand name or client name.
	...
	public function delete()
	{
		...
		if ($categoryId)
		{
			$allow = $user->authorise('core.delete', 'com_batbrands.category.' . (int) $categoryId);
		}
		else
		{
			$allow = $user->authorise('core.delete', 'com_batbrands');
		}

		if ($allow)
		{
			// Delete tracks from this batbrand
			$db    = $this->getDbo();
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__batbrand_tracks'));
			...
			$query->where('batbrand_id IN (SELECT id FROM ' . $db->quoteName('#__batbrands') . ' WHERE ' . $where . ')');
	...
	public function getBaseName()
	{
		if (!isset($this->basename))
		{
			...
			if ($type > 0)
			{
				...
				$typeName = JText::_('COM_BATBRANDS_TYPE' . $type);
	...
	protected function getCategoryName()
	{
		...
		return JText::_('COM_BATBRANDS_NOCATEGORYNAME');
	}
	...
	protected function getClientName()
	{
		...
		if ($clientId)
		{
			...
			$query = $db->getQuery(true)
				->select('name')
				->from($db->quoteName('#__batbrand_clients'))
		...
		return JText::_('COM_BATBRANDS_NOCLIENTNAME');
	}
	...
	public function getContent()
	{
		...
		{
			$this->content = '"' . str_replace('"', '""', JText::_('COM_BATBRANDS_HEADING_NAME')) . '","'
				. str_replace('"', '""', JText::_('COM_BATBRANDS_HEADING_CLIENT')) . '","'
				. str_replace('"', '""', JText::_('JCATEGORY')) . '","'
				. str_replace('"', '""', JText::_('COM_BATBRANDS_HEADING_TYPE')) . '","'
				. str_replace('"', '""', JText::_('COM_BATBRANDS_HEADING_COUNT')) . '","'
				. str_replace('"', '""', JText::_('JDATE')) . '"' . "\n";

			foreach ($this->getItems() as $item)
			{
				$this->content .= '"' . str_replace('"', '""', $item->batbrand_name) . '","'
					. str_replace('"', '""', $item->client_name) . '","'
					. str_replace('"', '""', $item->category_title) . '","'
					. str_replace('"', '""', ($item->track_type == 1 ? JText::_('COM_BATBRANDS_IMPRESSION') : JText::_('COM_BATBRANDS_CLICK'))) . '","'
			...
			if ($this->getState('compressed'))
			{
				...
				$ziproot = $app->get('tmp_path') . '/' . uniqid('batbrands_tracks_') . '.zip';

				// Run the packager
				jimport('joomla.filesystem.folder');
				jimport('joomla.filesystem.file');
				$delete = JFolder::files($app->get('tmp_path') . '/', uniqid('batbrands_tracks_'), false, true);
				...
				if (!empty($delete))
				{
					if (!JFile::delete($delete))
					{
						...
						$this->setError(JText::_('COM_BATBRANDS_ERR_ZIP_DELETE_FAILURE'));
				...
				{
					$this->setError(JText::_('COM_BATBRANDS_ERR_ZIP_ADAPTER_FAILURE'));

					return false;
				}
				elseif (!$packager->create($ziproot, $files))
				{
					$this->setError(JText::_('COM_BATBRANDS_ERR_ZIP_CREATE_FAILURE'));
