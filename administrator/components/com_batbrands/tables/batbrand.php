 ...
 * @since  1.5
 */
class BatbrandsTableBatbrand extends JTable
{
	/**
	 * Constructor
	 *
	...
	{
		parent::__construct('#__batbrands', 'id', $db);

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_batbrands.batbrand'));
	...
	public function bind($array, $ignore = array())
	{
		if (isset($array['params']) && is_array($array['params']))
		{
			...
			{
				$this->setError(JText::sprintf('JLIB_DATABASE_ERROR_NEGATIVE_NOT_PERMITTED', JText::_('COM_BATBRANDS_FIELD_WIDTH_LABEL')));
			...
			{
				$this->setError(JText::sprintf('JLIB_DATABASE_ERROR_NEGATIVE_NOT_PERMITTED', JText::_('COM_BATBRANDS_FIELD_HEIGHT_LABEL')));
	...
	public function store($updateNulls = false)
	{
		...
		if (empty($this->id))
		{
			...
			if ($purchaseType < 0 && $this->cid)
			{
				...
				$client = JTable::getInstance('Client', 'BatbrandsTable', array('dbo' => $db));
			...
			if ($purchaseType < 0)
			{
				$purchaseType = JComponentHelper::getParams('com_batbrands')->get('purchase_type');
			}
		...
		{
			// Get the old row
			/** @var BatbrandsTableBatbrand $oldrow */
			$oldrow = JTable::getInstance('Batbrand', 'BatbrandsTable', array('dbo' => $db));
			...
			$table = JTable::getInstance('Batbrand', 'BatbrandsTable', array('dbo' => $db));

			if ($table->load(array('alias' => $this->alias, 'catid' => $this->catid)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_BATBRANDS_ERROR_UNIQUE_ALIAS'));
	...
	public function stick($pks = null, $state = 1, $userId = 0)
	{
		...
		/** @var BatbrandsTableBatbrand $table */
		$table = JTable::getInstance('Batbrand', 'BatbrandsTable');
